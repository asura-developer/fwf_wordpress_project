...
/*
Plugin Name: TheGem Demo Import (for WPBackery)
...
function thegem_importer_scripts($hook) {
	if($hook == 'thegem_page_thegem-importer') {
...
<div class="wrap">
	...
	<?php elseif(!thegem_importer_get_purchase()) : ?>
		<?php if(!defined('ENVATO_HOSTED_SITE')) : ?>
			<p class="error-message"><?php printf(__('Please enter purchase code in <a href="%s">Theme Options</a>', 'thegem-importer'), admin_url('admin.php?page=thegem-theme-options#activation')); ?></p>
		<?php endif; ?>
	</div>
	<?php else : ?>
...
function thegem_importer_check_purchase_code () {
	$response_p = wp_remote_get(add_query_arg(array('code' => thegem_importer_get_purchase(), 'site_url' => get_site_url()), 'http://democontent.codex-themes.com/av_validate_code'.(defined('ENVATO_HOSTED_SITE') ? '_envato' : '').'.php'), array('timeout' => 20));
	if(!is_wp_error($response_p)) {
		...
		if(is_array($rp_data) && isset($rp_data['result']) && $rp_data['result'] && isset($rp_data['item_id']) && $rp_data['item_id'] === '16061685') {
			...
			if(!defined('ENVATO_HOSTED_SITE')) {
					echo json_encode(array('status' => 0, 'content' => '<p class="error-message">'.sprintf(__('Purchase code verification failed. <a href="%s">Activate TheGem</a>', 'thegem-importer'), admin_url('admin.php?page=thegem-theme-options#activation')).'</p>'));
			} else {
...
function thegem_importer_get_imports_list () {
	$response = wp_remote_get(esc_url('http://democontent.codex-themes.com/sites-democontent/imports.php'), array('timeout' => 20));
...
function thegem_importer_get_import_step_1() {
	$response = wp_remote_get(esc_url('http://democontent.codex-themes.com/sites-democontent/'.$_REQUEST['import'].'/import.json'), array('timeout' => 20));
	if(!is_wp_error($response)) {
		$rp_data = json_decode($response['body'], true);
		if(is_array($rp_data)) {
			ob_start();
			...
			$import_data['image'] = esc_url('http://democontent.codex-themes.com/sites-democontent/'.$_REQUEST['import'].'/screenshot.jpg');
			$sliders = $import_data['sliders'];
			foreach($sliders as $alias => $filename) {
				$sliders[$alias] = esc_url('http://democontent.codex-themes.com/sites-democontent/'.$_REQUEST['import'].'/sliders/'.$filename);
			}
...
<div class="thegem-importer-wrap thegem-importer-step-1">
	...
	<?php endif; ?>
	<?php if(!empty($imports_data[$_REQUEST['import']]['woocommerce']) && !function_exists('WC')) : ?>
		<div class="import-select-title"><?php _e('Warning', 'thegem-importer'); ?></div>
		<div class="import-select-warning"><?php _e('WooCommerce plugin is required. Please install it.', 'thegem-importer'); ?></div>
		<div class="submit-buttons">
			<button type="button" class="cancel-import"><?php _e('Cancel', 'thegem-importer'); ?></button>
			<a href="<?php echo admin_url('plugin-install.php?s=woocommerce&tab=search&type=term'); ?>" class="submit-import"><?php _e('Install Plugin', 'thegem-importer'); ?></a>
		</div>
	...
	<form class="import-data-select <?php echo esc_attr($_REQUEST['import_type']); ?>">
		...
		<?php
			if($_REQUEST['import_type'] == 'part') {
				$resp_content = wp_remote_get(esc_url('http://democontent.codex-themes.com/sites-democontent/'.$_REQUEST['import'].'/content/content_singles.json'), array('timeout' => 20));
				$resp_links = wp_remote_get(esc_url('http://democontent.codex-themes.com/sites-democontent/'.$_REQUEST['import'].'/posts_links.json'), array('timeout' => 20));
...
function thegem_importer_get_import_step_3() {
	...
	die(-1);
		...
		wp_remote_get(esc_url(add_query_arg($import_selection, 'http://democontent.codex-themes.com/sites-democontent/statistic.php'), null, ''), array('timeout' => 20));
		$content_file_temp = download_url('http://democontent.codex-themes.com/sites-democontent/'.$import_selection['import'].'/content/content.xml');
		$attachments_file_temp = download_url('http://democontent.codex-themes.com/sites-democontent/'.$import_selection['import'].'/content/content_attachments.json');
		$options_file_temp = download_url('http://democontent.codex-themes.com/sites-democontent/'.$import_selection['import'].'/theme_options.json');
		$page_settings_file_temp = download_url('http://democontent.codex-themes.com/sites-democontent/'.$import_selection['import'].'/page_settings.json');
		$menu_file_temp = download_url('http://democontent.codex-themes.com/sites-democontent/'.$import_selection['import'].'/content/content_menu.xml');
		$widgets_file_temp = download_url('http://democontent.codex-themes.com/sites-democontent/'.$import_selection['import'].'/widgets.json');
		$forms_file_temp = download_url('http://democontent.codex-themes.com/sites-democontent/'.$import_selection['import'].'/content/content_contact_forms.json');

		if( is_wp_error( $content_file_temp ) || is_wp_error( $attachments_file_temp ) || is_wp_error( $options_file_temp ) || is_wp_error( $page_settings_file_temp ) || is_wp_error( $menu_file_temp ) || is_wp_error( $widgets_file_temp ) || is_wp_error( $forms_file_temp )) {
...
function thegem_importer_update_images() {
	...
	$query = new WP_Query( array('post_status' => 'any', 'post_type' => array('thegem_title', 'thegem_footer'), 'posts_per_page'=>-1) );
	while ( $query->have_posts() ) {
		$query->the_post();
		$p = get_post();
		thegem_importer_fix_post_meta($p);
			...
			if(function_exists('WC') && !empty($import_data['woocommerce'])) {
				...
				if(!empty($import_data['woocommerce']['wishlist_page'])) {
					...
					if(!empty($wishlist_page_id->ID)) {
						update_option('woocommerce_wishlist_page_id', $wishlist_page_id->ID);
					}
				}
...
<div class="thegem-importer-wrap thegem-importer-error">
	...
	<div class="import-error-info">
		<?php if($error == 'ajax') : ?>
			<div class="import-error-title"><?php _e('Problem occured by processing the ajax request. Possible reasons for that:', 'thegem-importer'); ?></div>
			<div class="import-error-text">
				<p><?php printf(__('1. Seems that your server settings do not correspond with the recommended server settings (check here <a href="%s" target="_blank">%s</a>). Please contact your hoster and ask them to adjust your settings.', 'thegem-importer'), 'https://codexthemes.ticksy.com/article/8024/', 'https://codexthemes.ticksy.com/article/8024/'); ?></p>
				<p><?php _e('2. No internet connection. Please check your internet connection and try again.', 'thegem-importer'); ?></p>
				<p><?php printf(__('Please check this points and re-try importing or <a href="%s" target="_blank">contact our support</a>.', 'thegem-importer'), 'https://codexthemes.ticksy.com/'); ?></p>
			</div>
		<?php elseif($error == 'connecting') : ?>
			...
			<div class="import-error-text">
				<p><?php _e('1. Firewall IP blocking is active. Please check the firewall settings on your host/server and deactivate IP blocking.', 'thegem-importer'); ?></p>
