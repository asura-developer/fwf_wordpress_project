...
var UpdraftCentral = function() {
	...
	this.init = function() {
		...
		// Load sites (cached) responses
		UpdraftCentral.get_sites_information().then(function(info) {
			self.sites_info = info;
		});
		...
		function load_site_creds(site) {
			...
			credentials.load_credentials(site).then(function(response) {
				...
				if (response.hasOwnProperty('request_filesystem_credentials')) {
					...
					if (sysfolders.hasOwnProperty(module+'s') && sysfolders[module+'s']) {
						show_form = true;
					}
				}
