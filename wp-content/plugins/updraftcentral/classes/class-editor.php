...
class UpdraftCentral_Editor {
	...
	public function enqueue_editor_scripts() {
		global $post, $post_type, $post_type_object, $wp_meta_boxes;
		...
		if ($editor_styles && current_theme_supports('editor-styles')) {
			foreach ($editor_styles as $style) {
				if (preg_match('~^(https?:)?//~', $style)) {
					...
					$is_valid_file = (function_exists('is_file')) ? is_file($file) : file_exists($file);
					if ($is_valid_file) {
						$styles[] = array(
	...
	public function intercept_request_data($response, $handler, $request) {
		...
		// Check whether we received an updraftcentral "uc_nonce", "uc_refIds" and "site_id" data. This also helps in preventing any overlap
		...
		if (empty($params['uc_nonce']) || empty($params['uc_refIds']) || empty($params['site_id'])) return $response;

		list($user_id, $post_id) = explode('|', base64_decode($params['uc_refIds']));
		if (empty($user_id) || empty($post_id)) return $response;

		// Verify the nonce submitted
		if (!wp_verify_nonce($params['uc_nonce'], 'updraftcentral-editpost-'.$post_id)) {
			return $response;
		}
		...
		$preloaded_data = $site_meta->get_site_meta($site_id, 'updraftcentral_editor_preloaded_data', true);
		...
		if (!empty($match_result)) {
			...
			switch ($request->get_method()) {
				case 'GET':
					...
					$response = $controller->prepare_remote_item_for_response($post, $request, $post_data['misc']);
					break;
				case 'PUT':
					...
					$command = ('pages' == $matches[1]) ? 'pages.save_page' : 'posts.save_post';
					...
					if (!is_wp_error($data) && !empty($data)) {
						...
						$site_meta->update_site_meta($site_id, 'updraftcentral_editor_preloaded_data', $preloaded_data);

						// Generate the response
						$controller = new UpdraftCentral_REST_Posts_Controller($post->post_type);
						$response = $controller->prepare_remote_item_for_response($post, $request, $data['misc']);
	...
	public function maybe_store_preloaded_data($params) {
		...
		if (!empty($data) && !empty($params['site_id'])) {
			...
			$site_id = $params['site_id'];
			$site_meta = $this->get_site_meta_instance();
			$site_meta->update_site_meta($site_id, 'updraftcentral_editor_preloaded_data', $data);
		}
	}
	...
	public function load_classic_editor($params) {
		global $wp_meta_boxes, $post_type_object, $post, $site_id, $uc_categories_meta_box, $uc_tags_meta_box, $post_data;
		...
		if (!empty($item)) {
			if ($post) {
				...
				if (!empty($item['featured_media'])) {
					$thumbnail_result = set_post_thumbnail($post, $item['featured_media']);
				...
				if ('page' === $post->post_type) add_filter('get_pages', array($this, 'load_remote_pages'));
				ob_start();
				do_meta_boxes($post->post_type, 'side', $post);
				$metaboxes = ob_get_contents();
				ob_end_clean();
				if ('page' === $post->post_type) remove_filter('get_pages', array($this, 'load_remote_pages'));
